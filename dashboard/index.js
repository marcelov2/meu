...
module.exports = client => {
    ...
    app.get("/dashboard", checkAuth, async (req,res) => {
      ...
      return res.redirect("/?error=" + encodeURIComponent("Faça login primeiro!"));
      if(!req.user.guilds)
      return res.redirect("/?error=" + encodeURIComponent("Incapaz de obter suas Servidor!"));
    ...
    app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
      ...
      if (!guild) return res.redirect("/dashboard?error=" + encodeURIComponent("Não é possível obter dados de informações do Servidor"));
      let member = guild.members.cache.get(req.user.id);
      if (!member) {
        try {
          member = await guild.members.fetch(req.user.id);
        } catch (err) {
          console.error(`Não foi possível buscar ${req.user.id} in ${guild.name}: ${err}`);
        }
      }
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("Não foi possível buscar, desculpe!"));
      if (!member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
        return res.redirect("/dashboard?error=" + encodeURIComponent("Você não tem permissão para fazer isso!"));
      }
    ...
    app.post("/dashboard/:guildID", checkAuth, async (req, res) => {
      ...
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("Não posso fornecer informações sobre você!"));
    ...
    app.get("/queuedashboard", checkAuth, async (req,res) => {
      ...
      return res.redirect("/?error=" + encodeURIComponent("Faça login primeiro!"));
      if(!req.user.guilds)
      return res.redirect("/?error=" + encodeURIComponent("Incapaz de obter suas Guildas!"));
