console.log(`Welcome to SERVICE HANDLER /--/ By ehe-1.marceloolivei10.repl.co /--/ Discord: Marcelo`.yellow);
...
module.exports = (client) => {
    ...
    client.distube
        ...
        try {
          ...
          collector.on('collect', async i => {
            ...
            if (i.customId == `1`) {
              ...
              if (!channel)
                return i.reply({
                  content: `${client.allEmojis.x} **Por favor, Entre a um canal de voz primeiro!**`,
                  ephemeral: true
                })
              ...
              if (!queue || !newQueue.songs || newQueue.songs.length == 0) {
                return i.reply({
                  content: `${client.allEmojis.x} Nada em Tocando ainda`,
                  ephemeral: true
                })
              }
              ...
              if (channel.id !== newQueue.voiceChannel.id)
                return i.reply({
                  content: `${client.allEmojis.x} **Por favor, Entre a um canal de voz primeiro! <#${channel.id}>**`,
                  ephemeral: true
                })
              ...
              if (newQueue.songs.length == 0) {
                //if its on autoplay mode, then do autoplay before leaving...
                  i.reply({
                    content: `⏹ **Parou de tocar e saiu do canal**\n> 💢 **Ação feita por**: \`${member.user.tag}\``
                  })
                  clearInterval(songEditInterval);
                  collector.stop()
                  edited = true;
              ...
              i.reply({
                content: `⏭ **Pulei para a próxima música!**\n> 💢 **Ação feita por**: \`${member.user.tag}\``,
              })
              collector.stop();
            }
            //stop
            ...
            if (i.customId == `4`) {
              ...
              i.reply({
                content: `${newQueue.autoplay ? `${client.allEmojis.check_mark} **Reprodução automática habilitada**`: `${client.allEmojis.x} **Reprodução automática desativada**`}\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
            }
            //Shuffle
            if(i.customId == `5`){
              ...
              //Send Success Message
              i.reply({
                content: `🔀 **Shuffled ${newQueue.songs.length} Musicas**\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
            }
            //Songloop
            if(i.customId == `6`){
              ...
              i.reply({
                content: `${newQueue.repeatMode == 1 ? `${client.allEmojis.check_mark} **Song-Loop ativado**`: `${client.allEmojis.x} **Song-Loop desativad**`}\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
              var data = receiveQueueData(client.distube.getQueue(queue.id), newQueue.songs[0])
              currentSongPlayMsg.edit(data).catch((e) => {
                //console.log(e.stack ? String(e.stack).grey : String(e).grey)
            ...
            if(i.customId == `7`){
              ...
              i.reply({
                content: `${newQueue.repeatMode == 2 ? `${client.allEmojis.check_mark} **Loop de fila habilitado**`: `${client.allEmojis.x} **Loop de fila desativado**`}\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
              var data = receiveQueueData(client.distube.getQueue(queue.id), newQueue.songs[0])
              currentSongPlayMsg.edit(data).catch((e) => {
                //console.log(e.stack ? String(e.stack).grey : String(e).grey)
            ...
            if(i.customId == `8`){
              ...
              i.reply({
                content: `⏩ **Avançou a musica por \`10 Segundos\`**\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
              var data = receiveQueueData(client.distube.getQueue(queue.id), newQueue.songs[0])
              currentSongPlayMsg.edit(data).catch((e) => {
                //console.log(e.stack ? String(e.stack).grey : String(e).grey)
            ...
            if(i.customId == `9`){
              ...
              collector.resetTimer({time: (newQueue.songs[0].duration - newQueue.currentTime) * 1000})
              i.reply({
                content: `⏪ **Rebobinou a música para \`10 Segundos\`**\n> 💢 **Ação feita por**: \`${member.user.tag}\``
              })
              var data = receiveQueueData(client.distube.getQueue(queue.id), newQueue.songs[0])
              currentSongPlayMsg.edit(data).catch((e) => {
                //console.log(e.stack ? String(e.stack).grey : String(e).grey)
      ...
      .on(`addSong`, (queue, song) => queue.textChannel.send({
        embeds: [
          ...
          .setTitle(`${client.allEmojis.check_mark} **Canção adicionada à fila!**`)
          .setDescription(`👍 Musica: [\`${song.name}\`](${song.url})  -  \`${song.formattedDuration}\``)
          .addField(`⌛ **Tempo estimado:**`, `\`${queue.songs.length - 1} song${queue.songs.length > 0 ? "s" : ""}\` - \`${(Math.floor((queue.duration - song.duration) / 60 * 100) / 100).toString().replace(".", ":")}\``)
      ...
      .on(`addList`, (queue, playlist) => queue.textChannel.send({
        embeds: [
          ...
          .setTitle(`${client.allEmojis.check_mark} **Playlist adicionada à fila!**`)
          .setDescription(`👍 Playlist: [\`${playlist.name}\`](${playlist.url ? playlist.url : ""})  -  \`${playlist.songs.length} Song${playlist.songs.length > 0 ? "s" : ""}\``)
          .addField(`⌛ **Tempo estimado:**`, `\`${queue.songs.length - - playlist.songs.length} song${queue.songs.length > 0 ? "s" : ""}\` - \`${(Math.floor((queue.duration - playlist.duration) / 60 * 100) / 100).toString().replace(".", ":")}\``)
          .addField(`🌀 **Duração da fila:**`, `\`${queue.formattedDuration}\``)
        ]
      }))
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    .setDescription(`Veja a [Fila no ** DASHBOARD ** Live!](https://hehe-1.marceloolivei10.repl.co/queue/${newQueue.id})`)
      ...
      .addField(`♾ Loop:`, `>>> ${newQueue.repeatMode ? newQueue.repeatMode === 2 ? `${client.allEmojis.check_mark}\` Fila\`` : `${client.allEmojis.check_mark} \`Musica\`` : `${client.allEmojis.x}`}`, true)
      .addField(`↪️ Autoplay:`, `>>> ${newQueue.autoplay ? `${client.allEmojis.check_mark}` : `${client.allEmojis.x}`}`, true)
      .addField(`❔ Download Da Musica:`, `>>> [\`Click aqui\`](${newTrack.streamURL})`, true)
      ...
      .setAuthor(`${newTrack.name}`, `https://c.tenor.com/KRaMRFp7TQkAAAAC/groovy.gif`, newTrack.url)
